...
public class ConsultMaterialsController implements ViewController {
	 ...
	 * 
	 */    
	public String QueryFixer(String term){	
		
			
		if(term.toUpperCase().equals("AND") || term.toUpperCase().equals("OR"))
			return "\"" + term.trim() + "\"";
		
		try {
			
			if(term.subSequence(0, 1).equals("\""))
				return term;
		...
		for (int i = 0; i < temp.length; i++)
			if(!temp[i].toUpperCase().equals("AND") & !temp[i].toUpperCase().equals("OR"))
				fixedQuery += temp[i] + " ";
