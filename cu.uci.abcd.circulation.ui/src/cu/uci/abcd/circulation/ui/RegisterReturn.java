...
public class RegisterReturn extends ContributorPage implements Contributor {
			...
			if (workerLoggin == null) {
				...
				Column columnReturn = new Column("return", registerReturn.getDisplay(), new TreeColumnListener() {
					...
					public void handleEvent(final TreeColumnEvent event) {
						...
						MessageDialogUtil.openConfirm(Display.getCurrent().getActiveShell(), MessageUtil.unescape(cu.uci.abos.core.l10n.AbosMessages.get().MESSAGE_QUESTION), MessageUtil.unescape(AbosMessages.get().MSG_USER_SUCURE_RETURN) + " " + entity.getLoanObject().getTitle() + " "
								+ AbosMessages.get().MSG_USER_SUCURE_ASSOCIATE + " " + entity.getLoanUser().fullName() + "?", new DialogCallback() {
								...
								if (returnCode == 0) {
									...
									try {
										...
										if (entity.getParenttransaction() != null) {
											...
											Transaction transactionParent = entity.getParenttransaction();
											transactionParent.setState(loanState);
											((AllManagementLoanUserViewController) controller).getManageTransaction().addTransaction(transactionParent);

											if (transactionParent.getParenttransaction() == null)
												moreParents = false;

											while (moreParents) {

												transactionParent = transactionParent.getParenttransaction();
												transactionParent.setState(loanState);
												((AllManagementLoanUserViewController) controller).getManageTransaction().addTransaction(transactionParent);
									...
									entity = (Transaction) event.entity.getRow();
				...
				TreeTableColumn columns[] = { new TreeTableColumn(16, 0, "getLoanObject.getTitle"), new TreeTableColumn(16, 1, "getLoanObject.getInventorynumber"), new TreeTableColumn(16, 2, "getLoanObject.getRecordType.getNomenclatorName"),
						new TreeTableColumn(16, 3, "getState.getNomenclatorName"), new TreeTableColumn(16, 4, "getLoanUser.fullName"), new TreeTableColumn(16, 5, "getLoanUser.getLoanUserCode"), };
	...
	public void searchTransaction(int page, int size) {
		...
		Page<Transaction> list = ((AllManagementLoanUserViewController) controller).getManageTransaction().findAllTransactionByRegisterReturn(params, library, listWorkerRooms, page, size, size, orderByStringTransactions);
