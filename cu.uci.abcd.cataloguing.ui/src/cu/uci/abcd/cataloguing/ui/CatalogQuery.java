...
import cu.uci.abcd.cataloguing.listener.EventKeySimpleRecordQuery;
import cu.uci.abcd.cataloguing.listener.EventSimpleRecordQuery;
...
public class CatalogQuery extends ContributorPage {
	...
	public String contributorName() {
		return MessageUtil.unescape(AbosMessages.get().CATALOG_QUERY);
	}
	...
	public Control createUIControl(Composite parent) {
		...
		text.setMessage(MessageUtil.unescape(AbosMessages.get().TEXT_SEARCH_THE_CATALOG));
		FormDatas.attach(text).atTopTo(father, 5).atLeftTo(bar, 10).withWidth(450).withHeight(30);

		Button accept = new Button(father, SWT.PUSH);
		accept.setText(AbosMessages.get().BUTTON_SEARCH);
		FormDatas.attach(accept).atTopTo(father, 20).atLeftTo(text, 10).withHeight(25);

		accept.addListener(SWT.Selection, new EventSimpleRecordQuery((ProxyController) controller, dataBaseName, father, text));

		text.addKeyListener(new EventKeySimpleRecordQuery((ProxyController) controller, dataBaseName, father, text));
	...
	@Override
	public String getID() {
		return CataloguingUtil.CATALOG_QUERY_CONTRIBUTION_ID;
	}
