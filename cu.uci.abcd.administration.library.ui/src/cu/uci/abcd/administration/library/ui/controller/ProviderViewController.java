...
public class ProviderViewController implements ViewController {
	...
	public void saveProviderJisis(List<Provider> allProviders, Library library) {
		try {
			...
			//A partir de aki esta dinamico
			List<String> worksheets = getAllManagementLibraryViewController()
					.getLibraryService().getWorksheetNames(databaseName, isisDefHome);
			
			int workSheetsCount = worksheets.size();
			for (int i = 0; i < workSheetsCount; i++) {
				String workSheetName = worksheets.get(i);
				
				WorksheetDef workSheetI = getAllManagementLibraryViewController()
						.getLibraryService().getWorksheet(workSheetName,
								databaseName, isisDefHome);
				
				WorksheetField field = null;
				
				switch (workSheetName) {
				
                case "Donación":
                	field = workSheetI.getFieldByTag(25);
					break;
					
                case "Canje":
                	field = workSheetI.getFieldByTag(31);
					break;	

				default:
					field = workSheetI.getFieldByTag(24);
					break;
				}
				...
				if(null != field){
					...
					getAllManagementLibraryViewController().getLibraryService()
					.updateWorksheet(workSheetI);
				}
				else{
					System.out.println("El campo del proveedor no existe");
				}
			}
