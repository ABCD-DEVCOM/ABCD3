...
public class RegisterEnrollmentFragment extends ContributorPage implements FragmentContributor {
	...
	public RegisterEnrollmentFragment(Enrollment enrolment, ViewController controller, boolean isPostgradeEdit, int dimension) {
	...
	public Control createUIControl(Composite shell) {
		...
		FormDatas.attach(msg).atTopTo(parent, 10).atLeft(0).withHeight(50).atRight(0);
		...
		int width = left.getParent().getParent().getParent().getParent().getBounds().width;

		FormDatas.attach(left).atLeft(10).atTopTo(enrollmentData).withWidth((int) (width / 2.5));

		adreesedToCombo = new Combo(left, SWT.READ_ONLY);
		painter.add(adreesedToCombo);
		controls.put("adreesedToCombo", adreesedToCombo);
		FormDatas.attach(adreesedToCombo).atTopTo(left, 15).atLeftTo(adreesedTo, 10).atRight(15).withWidth(width / 4).withHeight(23);
		FormDatas.attach(adreesedTo).atRightTo(adreesedToCombo, 5).atLeft(5).atTopTo(left, 20);
		painter.reset();
		quantity = new Label(left, SWT.NONE);
		painter.add(quantity);
		FormDatas.attach(quantity).atTopTo(adreesedToCombo, 15).atRightTo(adreesedToCombo, 5);
		...
		validatorPrincipal.applyValidator(quantityText, "quantityText", DecoratorType.REQUIRED_FIELD, true);
		FormDatas.attach(quantityText).atTopTo(adreesedToCombo, 15).atLeftTo(adreesedTo, 5).atRight(15).withHeight(13);
		validatorPrincipal.applyValidator(quantityText, "quantityNumber", DecoratorType.NUMBER_ONLY_NOT_START_WITH_CERO, true);
		right = new Composite(parent, SWT.NONE);
		painter.addComposite(right, Percent.W50);
		controls.put("right", right);
		FormDatas.attach(right).atRight(15).atTopTo(enrollmentData).atLeftTo(left);
		...
		FormDatas.attach(firstUbicationToCombo).atTopTo(right, 15).atRight(15).withWidth((int) (width / 4.1)).withHeight(23);

		firstUbication = new Label(right, SWT.NONE | SWT.WRAP);
		controls.put("firstUbication", firstUbication);
		painter.add(firstUbication);
		FormDatas.attach(firstUbication).atTopTo(right, 15).atRightTo(firstUbicationToCombo, 5);
		...
		FormDatas.attach(secondUbicationToCombo).atTopTo(firstUbicationToCombo, 15).atRight(15).atLeftTo(firstUbication, 5).withHeight(23);

		secondUbication = new Label(right, SWT.NONE | SWT.WRAP);
		controls.put("secondUbication", secondUbication);
		painter.add(secondUbication);
		FormDatas.attach(secondUbication).atTopTo(firstUbicationToCombo, 20).atRightTo(secondUbicationToCombo, 5);
		...
		FormDatas.attach(thirdUbicationToCombo).atTopTo(secondUbicationToCombo, 15).atLeftTo(firstUbication, 5).atRight(15).withHeight(23);

		thirdUbication = new Label(right, SWT.NONE | SWT.WRAP);
		thirdUbication.setText(MessageUtil.unescape(AbosMessages.get().LABEL_STUDY_OR_INVESTIGATION_CENTER) + "sf \n df");
		controls.put("thirdUbication", thirdUbication);
		painter.add(thirdUbication);
		FormDatas.attach(thirdUbication).atTopTo(secondUbicationToCombo, 20).atRightTo(thirdUbicationToCombo, 5);
		...
		FormDatas.attach(programCombo).atTopTo(right, 15).atLeftTo(firstUbication, 5).atRight(13).withHeight(23);
		...
		FormDatas.attach(programNameText).atTopTo(programCombo, 15).atLeftTo(program, 5).atRight(15).withHeight(13);

		programName = new Label(right, SWT.NONE | SWT.WRAP);
		controls.put("programName", programName);
		painter.add(programName);
		FormDatas.attach(programName).atTopTo(programCombo, 20).atRightTo(programNameText, 5);
		painter.reset();
		...
		adreesedToCombo.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				int selectedAddressedToUserTypeIndex = adreesedToCombo.getSelectionIndex();
				if (selectedAddressedToUserTypeIndex > 0) {
					addressedTo = (Nomenclator) UiUtils.getSelected(adreesedToCombo);
		...
		initialize(adreesedToCombo, ((LibraryViewController) controller).getAllManagementLibraryViewController().getLibraryService().findNomenclatorIdOrderByCode(library.getLibraryID(), Nomenclator.ADDRESSED_TO_USER_TYPE));

		validatorPrincipal.applyValidator(adreesedToCombo, "adreesedToCombo", DecoratorType.REQUIRED_FIELD, true);
	...
	private void LoadEnrolmentDataData() {
		...
		if (enrolment != null) {
			...
			Nomenclator nomenclator = (Nomenclator) UiUtils.getSelected(adreesedToCombo);
			loadAddressedtoData(nomenclator);
			UiUtils.selectValue(firstUbicationToCombo, enrolment.getFirstUbication());
			UiUtils.selectValue(secondUbicationToCombo, enrolment.getSecondUbication());
			UiUtils.selectValue(thirdUbicationToCombo, enrolment.getThirdUbication());
			UiUtils.selectValue(programCombo, enrolment.getProgramm());
			programNameText.setText((enrolment.getProgramName() == null) ? "" : enrolment.getProgramName().toString());
		}
	...
	public void hideUbications() {
		...
		validator.decorationFactory.removeControlDecoration("firstUbicationToComboRequiredOne");
		secondUbicationToCombo.setVisible(false);
		validator.decorationFactory.removeControlDecoration("secondUbicationToComboRequiredOne");
		thirdUbicationToCombo.setVisible(false);
		validator.decorationFactory.removeControlDecoration("thirdUbicationToComboRequiredOne");
		programCombo.setVisible(false);
		validator.decorationFactory.removeControlDecoration("programComboRequiredOne");
		programNameText.setVisible(false);
		validator.decorationFactory.removeControlDecoration("programNameTextRequiredOne");
		validator.decorationFactory.removeControlDecoration("programNameTextRequiredOne1");
	...
	public void loadAddressedtoData(Nomenclator addressedTo) {
		if (addressedTo != null) {
			...
			if (addressedTo.equals(pregradeStudent)) {
				firstUbication.setText(MessageUtil.unescape(AbosMessages.get().LABEL_FACULTY));
				secondUbication.setText(MessageUtil.unescape(AbosMessages.get().LABEL_CARRERA));
				...
				validator.applyValidator(firstUbicationToCombo, "firstUbicationToComboRequiredOne", DecoratorType.REQUIRED_FIELD, true);
				validator.applyValidator(secondUbicationToCombo, "secondUbicationToComboRequiredOne", DecoratorType.REQUIRED_FIELD, true);
				...
				if (addressedTo.equals(postgradeStudent)) {
					...
					programName.setText(MessageUtil.unescape(AbosMessages.get().LABEL_PROGRAM_NAME));
					...
					validator.applyValidator(programCombo, "programComboRequiredOne", DecoratorType.REQUIRED_FIELD, true);

					validator.applyValidator(programNameText, "programNameTextRequiredOne", DecoratorType.REQUIRED_FIELD, true);

					validator.applyValidator(programNameText, "programNameTextRequiredOne1", DecoratorType.ALPHA_NUMERICS_SPACES, true, 50);
					...
					if (addressedTo.equals(professor)) {
						...
						firstUbication.setText(MessageUtil.unescape(AbosMessages.get().LABEL_FACULTY));
						secondUbication.setText(MessageUtil.unescape(AbosMessages.get().LABEL_DEPARTMENT));
						thirdUbication.setVisible(true);
						thirdUbication.setText(MessageUtil.unescape(AbosMessages.get().LABEL_STUDY_OR_INVESTIGATION_CENTER));
						program.setText("");
						programName.setText("");
						...
						loadCombo(firstUbicationToCombo, Nomenclator.FACULTY);
						loadCombo(secondUbicationToCombo, Nomenclator.DEPARTMENT);
						List<Nomenclator> listInvestigationCenter = ((LibraryViewController) controller).findNomenclatorByCode(library.getLibraryID(), Nomenclator.INVESTIGATION_CENTER);
						List<Nomenclator> listStudyCenter = ((LibraryViewController) controller).findNomenclatorByCode(library.getLibraryID(), Nomenclator.STUDY_CENTER);
						listInvestigationCenter.addAll(listStudyCenter);
						loadCombo(thirdUbicationToCombo, listInvestigationCenter);

						validator.applyValidator(firstUbicationToCombo, "firstUbicationToComboRequiredOne", DecoratorType.REQUIRED_FIELD, true);
						validator.applyValidator(secondUbicationToCombo, "secondUbicationToComboRequiredOne", DecoratorType.REQUIRED_FIELD, true);
						validator.applyValidator(thirdUbicationToCombo, "thirdUbicationToComboRequiredOne", DecoratorType.REQUIRED_FIELD, true);
						...
						if (addressedTo.equals(investigator)) {
							...
							firstUbication.setText(MessageUtil.unescape(AbosMessages.get().LABEL_FACULTY));
							secondUbication.setText(MessageUtil.unescape(AbosMessages.get().LABEL_INVESTIGATION_CENTER));
							thirdUbication.setText("");
							program.setText("");
							programName.setText("");
							...
							loadCombo(firstUbicationToCombo, Nomenclator.FACULTY);
							loadCombo(secondUbicationToCombo, Nomenclator.INVESTIGATION_CENTER);
							...
							firstUbication.setText(MessageUtil.unescape(AbosMessages.get().LABEL_UBICATION));
	...
	private void loadCombo(Combo combo, long nomenclatorType) {
		initialize(combo, ((LibraryViewController) controller).findNomenclatorByCode(library.getLibraryID(), nomenclatorType));
	}

	@Override
	public void l10n() {
		enrollmentData.setText(MessageUtil.unescape(AbosMessages.get().LABEL_ENROLLMENT_DATA));
		adreesedTo.setText(MessageUtil.unescape(AbosMessages.get().LABEL_DIRECTED_TO));
		program.setText(MessageUtil.unescape(AbosMessages.get().LABEL_PROGRAM_TYPE));
		programName.setText(MessageUtil.unescape(AbosMessages.get().LABEL_PROGRAM_NAME));
