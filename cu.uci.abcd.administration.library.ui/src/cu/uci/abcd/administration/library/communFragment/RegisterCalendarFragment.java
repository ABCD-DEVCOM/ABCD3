...
public class RegisterCalendarFragment implements FragmentContributor {
	...
	public RegisterCalendarFragment(ViewController controller, int dimension, int option) {
	...
	public Control createUIControl(Composite shell) {
		...
		validator.applyValidator(titleText, "titleTextAlphaNumericSpaces", DecoratorType.ALPHA_NUMERICS_SPACES, true, 50);
	...
	public void l10n() {
		...
		titleLabel.setText(MessageUtil.unescape(MessageUtil.unescape(AbosMessages.get().LABEL_TITLE)));
		descriptionLabel.setText(MessageUtil.unescape(AbosMessages.get().LABEL_DESCRIPTION));
		holidayData.setText(MessageUtil.unescape(AbosMessages.get().LABEL_HOLIDAY_DATA));
		if (option == 1) {
			registerCalendarLabel.setText(MessageUtil.unescape(AbosMessages.get().LABEL_REGISTER_HOLIDAY));
		} else {
			registerCalendarLabel.setText(MessageUtil.unescape(AbosMessages.get().LABEL_UPDATE_CALENDAR));
		}
	}
