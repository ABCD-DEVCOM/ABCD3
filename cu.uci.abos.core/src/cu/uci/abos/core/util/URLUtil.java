...
public class URLUtil {
	...
	public static void download(String path, ReportType type) {

		try {
			UrlLauncher launcher = RWT.getClient().getService(UrlLauncher.class);
			launcher.openURL(createDownloadUrlOPAC(type, path));
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	...
	private static String createDownloadUrlOPAC(ReportType reportType, String fileName) {
		StringBuilder url = new StringBuilder();		
		url.append(fileName);
		return url.toString();
	}
