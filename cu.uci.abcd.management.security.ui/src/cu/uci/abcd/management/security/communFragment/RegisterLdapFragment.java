...
public class RegisterLdapFragment implements FragmentContributor {
	...
	public Control createUIControl(Composite parent) {
		...
		validator.applyValidator(ldapHostText, "ldapUrlText1", DecoratorType.IP, true, 20);

		validator.applyValidator(ldapHostText, "ldapUrlText", DecoratorType.REQUIRED_FIELD, true);
		...
		validator.applyValidator(ldapPortText, "ldapPortText1", DecoratorType.PORT, true, 4);

		validator.applyValidator(ldapPortText, "ldapPortText", DecoratorType.REQUIRED_FIELD, true);
		...
		validator.applyValidator(ldapDomainText, "ldapDomainText1", DecoratorType.DOMAIN, true, 20);

		validator.applyValidator(ldapDomainText, "ldapDomainText", DecoratorType.REQUIRED_FIELD, true);
	...
	public void saveLdap(ViewController controller, Ldap ldap, IVisualEntityManager manager, CRUDTreeTable table) {
		...
		if (existLdapWithDomain == null || (ldap != null && ldap.getLdapID() == existLdapWithDomain.getLdapID())) {
			...
			RetroalimentationUtils.showErrorShellMessage(AbosMessages.get().MESSAGE_LDAP + MessageUtil.unescape(": ") + domain);
		}
	}
